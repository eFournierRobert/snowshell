...
Test(directory_testing, change_to_non_existant_dir, .init=setup) {
    ...
    cr_assert_str_eq(buf, expected, "Error message did not match the expected error message.");
    cr_assert_str_eq(current_dir, real_current_dir, "Directory shouldn't have change on invalid directory");
}

Test(directory_testing, change_to_a_file, .init=not_dir_setup, .fini=not_dir_teardown) {
    char *argv[3] = {0};
    argv[0] = "cd";
    argv[1] = test_filename;
    argv[2] = NULL;
    char current_dir[PATH_MAX];
    cr_assert_not_null(getcwd(current_dir, sizeof(current_dir)), "getcwd returned NULL.");
